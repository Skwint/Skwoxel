	...
	namespace simplify
	{
		...
		bool flipped(godot::Vector3 p, int i0, int i1, Vertex& v0, Vertex& v1, lotsa<int>& deleted)
		{
			...
			for (int k = 0; k < v0.tcount; ++k)
			{
				...
				godot::Vector3 d1 = vertices[id1].p - p; d1.normalize();
				godot::Vector3 d2 = vertices[id2].p - p; d2.normalize();
		...
		void update_mesh(int iteration)
		{
			...
			if (iteration == 0)
			{
				...
				{
					// Calc Edge Error
					Triangle& t = triangles[i]; godot::Vector3 p;
		...
		double calculate_error(int id_v1, int id_v2, godot::Vector3& p_result)
		{
			...
			else
			{
				...
				godot::Vector3 p1 = vertices[id_v1].p;
				godot::Vector3 p2 = vertices[id_v2].p;
				godot::Vector3 p3 = (p1 + p2) / 2;
